...
function getPaths() {
  ...
  return {
    ...
    // Keep the existing path used by bin/svpn.js for backward compatibility.
...
function renameProfile(paths, selector, newName) {
  ...
  console.log(`订阅方案已重命名：${oldName} -> ${name}`);
}
...
function deleteProfile(paths, selector, options = {}) {
  ...
  writeProfiles(paths, nextProfiles);
  ...
  if (isActive) {
    ...
    // Keep the active Clash config file as-is so a running backend is not broken.
    // The user can run `svpn profile use ...` or `svpn import ...` to select another saved profile.
    writeJson(paths.settingsFile, settings);
  }

  console.log(`订阅方案已删除：${profile.name || profile.id}`);
  if (isActive) {
    console.log('提示：删除的是当前方案；当前运行配置文件已保留，但已不再绑定到已保存 profile。');
  }
}
...
function main() {
  ...
  if (command === 'rename') {
    ...
    const newName = argv.slice(2).join(' ');
    return renameProfile(paths, selector, newName);
  }
  if (command === 'delete' || command === 'remove' || command === 'rm') {
    const selector = argv[1];
    const yes = argv.includes('--yes') || argv.includes('-y');
    return deleteProfile(paths, selector, { yes });
  }
